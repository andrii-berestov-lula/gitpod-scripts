...
module UserConfig =
    ...
    let ask () =
        ...
        { Email = username
          Password = password
          KratosUrl = if kratosUrl.Length > 0 then kratosUrl else "http://localhost:4433"
          }
...
module Ory =
    ...
    let private getToken (url:string) =
        task {
            ...
            let! ssr = JsonSerializer.DeserializeAsync<SelfServiceResponse>(resStream, JsonSerializerOptions(PropertyNameCaseInsensitive = true))
            return match ssr.Id with
                    | null -> None
                    | x -> Some x
        }
    let private getRegisterToken user = getToken $"{user.KratosUrl}/self-service/registration/api"   
    let private getLoginToken user = getToken $"{user.KratosUrl}/self-service/login/api"
    ...
    let register user =
        try 
            match getRegisterToken user |> Async.AwaitTask |> Async.RunSynchronously with
            | None -> printfn "Could not retrieve flowId"; Environment.Exit(1)
            | Some x ->
                match registerUser x user |> Async.AwaitTask |> Async.RunSynchronously with
                | None -> printfn $"Could not register user: %s{user.Email}" ; Environment.Exit(2)
                | Some _ -> printfn $"Successfully registered user: %s{user.Email}"; Environment.Exit(0)
        with
        | :? AggregateException ->
    ...
    let login user =
        try 
            match getLoginToken user |> Async.AwaitTask |> Async.RunSynchronously with
            | None -> printfn "Could not retrieve flowId"; Environment.Exit(1)
            | Some x ->
                match loginUser x user |> Async.AwaitTask |> Async.RunSynchronously with
                | None -> printfn $"Could not log in user: %s{user.Email}" ; Environment.Exit(2)
                | Some _ -> printfn $"Successfully logged in user: %s{user.Email}"; Environment.Exit(0)
        with
        | :? AggregateException ->
            printfn "Gitpod should have gone timeout. Please start it once again in Browser."
            Environment.Exit(3)
...
let main argv =
    ...
    | ["register"] ->
                    ...
                    UserConfig.ask () |> UserConfig.save defaultConfigPath |> Ory.register
    | [] -> 
            if File.Exists defaultConfigPath
            then
                ...
                Environment.Exit(1)
    | [x] -> if File.Exists defaultConfigPath
             then
                Ory.login <| UserConfig.read defaultConfigPath
                runOathkeeper x
             else
